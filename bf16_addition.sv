...
module bf16_add_tb ();
    ...
    // --------------------------------------------------------
    // 1. Declare signals to connect to the DUT
    // --------------------------------------------------------
    logic [15:0] a;
    logic [15:0] b;
    logic [15:0] result;

    // 2. Instantiate the adder module
    bf16_add dut (
        .a(a),
        .b(b),
        .result(result)
    ...
    // --------------------------------------------------------
    initial begin
        // Open the ADDITION text file generated by the Python Golden Model
        file_handle = $fopen("Top_Level_Verification/add_vectors.txt", "r");

        if (file_handle == 0) begin
            $display("FATAL ERROR: Could not open add_vectors.txt! Check your file path.");
            $finish;
        end

        $display("==================================================");
        $display("Starting BF16 Adder File-Based Testbench...");
        $display("==================================================");

        // Loop through the file line by line
        while (!$feof(file_handle)) begin
            
            // Read A, B, and the Expected Sum
            scan_return = $fscanf(file_handle, "%h %h %h\n", a, b, expected_result);

            if (scan_return == 3) begin
                
                #10; // Wait for the massive adder logic block to settle
        ...
        if (error_count == 0) begin
            $display("SUCCESS: All %0d vectors passed! The adder is flawless.", test_count);
        end else begin
...
// 	bf16_add dut (.a(A), .b(B), .result(result));
